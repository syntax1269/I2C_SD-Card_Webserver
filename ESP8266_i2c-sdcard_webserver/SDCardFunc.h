...
- XXXGetCrdTyp() Queries the I2C SD card module for its type using the 'Q' command and prints the interpreted type (SDv1, SDv2, SDHC/SDXC, MMC, Unknown) to the Serial monitor. No return value. (Likely a debug/test version).
- GetCrdTyp() Queries the I2C SD card module for its type using the 'Q' command. Returns a const char* pointer to a string literal representing the card type ("SDv1", "SDv2", "SDHC/SDXC", "MMC", "Unknown or error", or an I2C error message). Uses F() macro for Serial output on errors.
